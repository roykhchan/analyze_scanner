...
import { VolatilityContractionRuleDaily } from "../../entities/rules/volatilty-contraction-rule-daily";
...
export class VcpQuery implements BaseQuery<void, TickerItem[]> {
    ...
    run(param: any): TickerItem[] {
        ...
        let vcpSpecDaily = new VolatilityContractionRuleDaily();
        ...
        for(let i = 0; i < this.tickerList.length; i++) {
            try{
                let critiria = min30PercentMonthlyPerformance
